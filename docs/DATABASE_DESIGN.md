...
```mermaid
erDiagram
    EMPLOYERS ||--o{ VACANCIES : "has"
    
    EMPLOYERS {
        integer employer_id PK "ID работодателя из hh.ru"
        varchar name "Название компании"
        varchar url "Ссылка на компанию"
        text description "Описание компании"
        varchar area "Регион"
        integer open_vacancies "Количество открытых вакансий"
    }
    
    VACANCIES {
        integer vacancy_id PK "ID вакансии из hh.ru"
        integer employer_id FK "ID работодателя"
        varchar name "Название вакансии"
        varchar url "Ссылка на вакансию"
        integer salary_from "Зарплата от"
        integer salary_to "Зарплата до"
        varchar currency "Валюта"
        text requirement "Требования"
        text responsibility "Обязанности"
        timestamp published_at "Дата публикации"
    }
```
...
#### 1. **Employers (Работодатели)**
   ...
   - Первичный ключ: `employer_id` (уникальный ID из hh.ru)
   - Атрибуты:
     - `name` - название компании (обязательное поле)
     - `url` - ссылка на страницу компании на hh.ru
     - `description` - описание компании
     - `area` - регион расположения компании
     - `open_vacancies` - количество открытых вакансий

#### 2. **Vacancies (Вакансии)**
   - Хранит информацию о вакансиях
   - Первичный ключ: `vacancy_id` (уникальный ID из hh.ru)
   - Внешний ключ: `employer_id` → `employers.employer_id`
   - Атрибуты:
     - `name` - название вакансии (обязательное поле)
     - `url` - ссылка на вакансию на hh.ru
     - `salary_from` / `salary_to` - диапазон зарплаты
     - `currency` - валюта зарплаты (RUR, USD, EUR)
...
- **Один ко многим (1:N)**: 
  - Один работодатель (`Employers`) может иметь множество вакансий (`Vacancies`)
  - Каждая вакансия принадлежит одному работодателю
  - Связь реализуется через внешний ключ `employer_id` в таблице `vacancies`
  - При удалении работодателя все его вакансии удаляются автоматически (ON DELETE CASCADE)
...
- Один работодатель → Много вакансий
- Много вакансий → Один работодатель
